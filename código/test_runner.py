...
pdf_path = "provas-e-gabaritos-unicamp-2026/1-fase-unicamp-2026/prova-q-x-1-fase-unicamp-2026.pdf"
...
gabarito_path = "provas-e-gabaritos-unicamp-2026/1-fase-unicamp-2026/gabarito-q-x-1-fase-unicamp-2026.pdf"
...
if os.path.exists(gabarito_path):
    ...
    print(f"Aviso: Gabarito não encontrado em {gabarito_path}")
...
if os.path.exists(questoes_dir):
    ...
    q46_file = f"{edital}_{ano}_{tipo_cor.replace('/', '-')}_46.json"

    if os.path.exists(os.path.join(questoes_dir, q46_file)):

        with open(os.path.join(questoes_dir, q46_file), "r", encoding="utf-8") as f:
            ...
            print(f"\n--- Validação da Questão {num} (Gabarito Oficial: C) ---")
        ...
        print(f"\nNota: {q46_file} não processada com IA no teste rápido (processadas apenas as 2 primeiras).")
