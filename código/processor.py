...
def detectar_edital_ano(pdf_path):
    ...
    # Detecção dinâmica do tipo/cor da prova
    tipo_prova = "Q-X"
    for cor in ["azul", "amarelo", "rosa", "verde", "cinza", "branco", "preto", "laranja"]:
        ...
        # Tenta encontrar padrões como "q-x", "r-y", "s-z", "t-w"
        match_letras = re.search(r"\b([a-z]-[a-z])\b", nome_arquivo)
        if match_letras:
            tipo_prova = match_letras.group(1).upper()
        else:
            # Padrão genérico de caderno ou prova seguido de uma palavra/letra
...
def extrair_gabarito(gabarito_path):
    ...
    tipo_match = re.search(r"PROVAS\s+([A-Za-z\s+e\s+\-]+)", texto, re.IGNORECASE)

    tipo_gabarito = tipo_match.group(1).strip() if tipo_match else "Desconhecido"
...
2. Tags conceituais: Retorne exclusivamente tópicos teóricos ou subtemas do assunto da questão (ex: "Termodinâmica", "Segunda Guerra Mundial", "Citologia"). É expressamente PROIBIDO incluir o nome do vestibular ("unicamp", "enem", etc.) ou matérias gerais ("física", "história", etc.) nas tags.
